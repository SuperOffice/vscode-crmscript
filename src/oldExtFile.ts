    ...
    public provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): Thenable<vscode.CompletionItem[]> {
        return new Promise(
            (resolve, reject) => {                
                ...
                for (let i in ejScriptIntellisense) {
                    var s = pluck(ejScriptIntellisense[i], ['text']);

                    items.push(new vscode.CompletionItem(s[0]));
                }
                resolve(items);
            }
        );
...
class CRMScriptHoverProvider implements vscode.HoverProvider {
    public provideHover (document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken): vscode.ProviderResult<vscode.Hover> {		
        var s = pluck(ejScriptIntellisense[0], ['help']);
